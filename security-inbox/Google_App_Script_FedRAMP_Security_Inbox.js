...
const CONFIG = {
  ...
  // Mailing list address that FedRAMP emails are routed through
  mailingListAddress: 'fedramp@paramify.com',
...
/**
 * Check if a message is genuinely from FedRAMP/GSA.
 * Handles both direct sends and emails routed through a mailing list,
 * where Gmail rewrites From: to the list address but preserves Reply-To.
 */
function isFromFedRAMP(message) {
  var from = message.getFrom();
  var replyTo = message.getReplyTo();

  return /@gsa\.gov/i.test(from)      ||
         /@fedramp\.gov/i.test(from)  ||
         /@gsa\.gov/i.test(replyTo)   ||
         /@fedramp\.gov/i.test(replyTo);
}
...
function checkEmergencyEmails() {
  var searchQuery = buildSearchQuery(
    '(from:fedramp_security@gsa.gov OR from:fedramp_security@fedramp.gov OR from:FedRAMP_Security@gsa.gov OR from:FedRAMP_Security@fedramp.gov OR from:' + CONFIG.mailingListAddress + ')',
  ...
  threads.forEach(function(thread) {
    ...
    if (!isFromFedRAMP(message)) {
      Logger.log('Skipping non-FedRAMP Emergency email from: ' + message.getFrom());
      return;
    }
...
function checkEmergencyTestEmails() {
  ...
  threads.forEach(function(thread) {
    ...
    if (!isFromFedRAMP(message)) {
      Logger.log('Skipping non-FedRAMP Emergency Test email from: ' + message.getFrom());
      return;
    }
...
function checkImportantEmails() {
  var searchQuery = buildSearchQuery(
    '(from:@fedramp.gov OR from:@gsa.gov OR from:' + CONFIG.mailingListAddress + ')',
    'subject:"Important"'
  );
  ...
  threads.forEach(function(thread) {
    ...
    if (!isFromFedRAMP(message)) {
      Logger.log('Skipping non-FedRAMP Important email from: ' + message.getFrom());
      return;
    }
...
function checkGeneralFedRAMPEmails() {
  ...
  threads.forEach(function(thread) {
    ...
    if (!isFromFedRAMP(message)) {
      Logger.log('Skipping non-FedRAMP General email from: ' + message.getFrom());
      return;
    }
